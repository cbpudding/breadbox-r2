...
void breadbox_log(breadbox_logsource_t source, breadbox_loglevel_t level, const char *format, va_list args) {
    printf("%11.3f %4s %4s ", (float)SDL_GetTicks() / 1000.0, LOGSOURCE_IDENTIFIERS[source], LOGLEVEL_IDENTIFIERS[level]);
    vprintf(format, args);
...
void breadbox_log_warning(breadbox_logsource_t source, const char *format, ...) {
    va_list args;
    va_start(args, format);
